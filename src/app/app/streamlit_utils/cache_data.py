...
def get_post_db():
    ...
    posts = post_db.execute(text("""
            select p."content", p.title, p.created_at, p.updated_at, p.author_name,
            d."name" as departement_name, d."number" as number_departement
...
def get_formation_db():
    ...
    formations = annuaire_db.execute(text("select * from partenaire.public.formations"))
